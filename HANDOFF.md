...
> **Latest documented release:** [v1.5.6](https://github.com/SlippedPenguin/mangolist/releases/tag/v1.5.6)
> **Working tree:** v1.5.6 polish pass: (1) Anime/Manga sub-tabs (Watchlist/Explore/Airing) and Profile tabs are now centered pill tabs via a shared CenteredPillTabs component — the left-aligned ScrollableTabRow is gone. (2) Watchlist category selection moved into a corner FilterList icon button in LibraryHeader (AniHyou style) with a status DropdownMenu; a three-line Sort icon adds sort modes (Recently updated / Title / Score / Progress / Tier). (3) Profile got an icon-driven header row (avatar + name + gear → Settings) and icon tabs (Person/History/BarChart/Settings). (4) Detail tracking card: the four stacked full-width buttons became three compact icon tiles (Tier/Notes/Score) and the redundant "Sync to AniList" button was removed — SyncWorker auto-pushes every local edit, so manual sync was redundant. (5) Every tab now keeps its own state (scroll, filter, sort) via rememberSaveableStateHolder — switching sub-tabs or bottom tabs and returning restores where you were instead of resetting.
